...
public class DepositCalculator {
    ...
    double calculateComplexPercent(double amount, double yearRate, int depositPeriod) {
        double amountBeforeRounding = amount * Math.pow((1 + yearRate / 12), 12 * depositPeriod);
        return roundOutAmount(amountBeforeRounding, 2);
    }

    double calculateSimplePercent(double amount, double yearRate, int depositPeriod) {
        return roundOutAmount(amount + amount * yearRate * depositPeriod, 2);
    }

    double roundOutAmount(double value, int places) {
